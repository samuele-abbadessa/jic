...
| WT-5 | chiuso | Merge two-worktree in `session end --merge` | Se il branch base è già checked-out nel worktree principale, git rifiuta il checkout (vincolo two-worktree). Valutare merge senza checkout o dal lato del worktree root principale. | Risolto dall'approccio b2 (merge nel worktree del base, niente checkout che collide) — commit `c74ae7a` | 2026-06-08 |
...
| WT-15 | aperto | Test integrazione root merge b2 | Verificare con repo temporanei che jic session end --merge dal chunk worktree aggiorni il branch di piano nel worktree dell'integration via merge in-place (casi: fast-forward, conflitto→abort, working tree sporco, base non checked-out→path legacy). Oggi coperto solo da unit test sull'helper findWorktreeForBranch. | correlata WT-11; origine: spec worktree-root-merge-propagation | |
| WT-16 | aperto | Rollback transazionale del root merge nel path legacy | Nel path legacy del root merge (mergeSessionBranches), se stageSubmodulePointers/commitSubmodulePointers falliscono DOPO checkout+merge, il repo root resta sul base branch con merge non committato e nessun rollback. Edge-case pre-esistente (non introdotto dal fix b2). Valutare cleanup/rollback su errore. | pre-esistente, emerso in review chunk 2; src-ts/commands/session.ts mergeSessionBranches | |
